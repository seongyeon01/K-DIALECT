...
class GPT(nn.Module):
    ...
    def forward(
        ...
        # if dialect_emb is not None:
        #     # shape: (B, D) -> (B, 1, D) -> broadcast to (B, T, D)
        #     text_emb = text_emb + dialect_emb.unsqueeze(1)
        ...
        if dialect_emb is not None:
            cond_latents = cond_latents + dialect_emb.unsqueeze(1).expand(-1, cond_latents.size(1), -1)
